...
import {
  ...
  SectionHeading,
  SectionLabel,
} from "@/components/reusableTitles";
...
import { ArrowRight, Flame, ChevronRight, Check } from "lucide-react";
...
export default function Home() {
  ...
  // Pre-calculate active plan keys to avoid repetition and scope issues within the map loop
  ...
  return (
    <main
      ...
      className="relative min-h-screen bg-[#0a0a0a] selection:bg-orange-500/20"
    >
      {/* Starfield background configurations */}
      <StarsBackground
        className="fixed inset-0 z-0 h-full w-full [filter:hue-rotate(-30deg)_saturate(1.4)]"
        style={{
          maskImage:
            "linear-gradient(to bottom, black 0%, black 15%, rgba(0,0,0,0.35) 45%, rgba(0,0,0,0.12) 65%, black 92%, black 100%)",
          WebkitMaskImage:
            "linear-gradient(to bottom, black 0%, black 15%, rgba(0,0,0,0.35) 45%, rgba(0,0,0,0.12) 65%, black 92%, black 100%)",
        }}
      />
      ...
      <section className="relative flex flex-col items-center overflow-hidden px-4 pb-24 pt-40 text-center">
        ...
        <h1 className="mx-auto max-w-3xl text-balance font-serif text-5xl leading-tight tracking-tight sm:text-5xl lg:text-7xl z-10">
          <GrayTitle>Describe the app.</GrayTitle>
          <br />
          <BlueTitle>Forge writes the code.</BlueTitle>
        </h1>

        <p className="mx-auto mt-6 max-w-xl text-balance text-base leading-relaxed text-white/40 z-10">
          Type what you want to build in plain language. Forge picks the stack,
          writes the code, and renders a live preview you can click through in
          seconds — no scaffolding, no config, no boilerplate to wire up
          yourself.
        </p>
          ...
          <div className="flex h-105">
                ...
                <div className="flex gap-2.5">
                  ...
                  <div className="rounded-2xl rounded-tl-sm bg-white/5 px-3.5 py-2.5">
                    <p className="text-xs text-white/60">
                      Setting up three columns — Todo, In Progress, Done — and
                      wiring up{" "}
                      <code className="text-orange-400/80">@dnd-kit/core</code>{" "}
                      for drag-and-drop between them…
                    </p>
                  </div>
                </div>
            ...
            <div className="flex flex-1 flex-col">
              <div className="flex items-center gap-1 border-b border-white/6 px-4">
                <button className="border-b-2 border-orange-400 px-3 py-2.5 text-xs text-white">
                  Preview
                </button>
      ...
      <section className="relative px-4 pb-32">
        <div className="mx-auto mb-14 max-w-5xl text-center">
          <SectionLabel>What Forge handles for you</SectionLabel>
          ...
          {FEATURES.map(({ icon: Icon, label, desc }) => (
            ...
            >
              <div className="mb-4 flex h-9 w-9 items-center justify-center rounded-lg border border-white/8 bg-white/4 group-hover:border-white/15 group-hover:bg-white/8">
                <Icon className="h-4 w-4 text-white/60 group-hover:text-orange-400/70" />
              </div>
      ...
      <section className="relative px-4 pb-32">
        <div className="mx-auto mb-14 max-w-3xl text-center">
          ...
          <SectionHeading gray="One prompt," blue="a working app." />
        </div>
          ...
          {PRICING_PLANS.map((plan) => {
            ...
            return (
              <div
                key={plan.key}
                className={cn(
                  "relative flex flex-col rounded-2xl border p-7 transition-all duration-300 backdrop-blur-md",
                  plan.featured
                    ? "border-orange-500/40 bg-orange-500/[0.03] shadow-lg shadow-orange-500/5 ring-1 ring-orange-500/20"
                    : "border-white/10 bg-black/40 hover:border-white/20",
                )}
              >
                {plan.featured && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                    <span className="rounded-full border border-orange-500/30 bg-black px-3 py-1 text-[11px] font-medium text-orange-400 tracking-wide shadow-sm">
                      Most popular
                    </span>
                  </div>
                )}
                    ...
                    <div key={f} className="flex items-center gap-2.5">
                      <div
                        className={cn(
                          ...
                          plan.featured ? "bg-orange-500/15" : "bg-white/8",
                        )}
                      >
                        <Check
                          className={cn(
                            "h-2.5 w-2.5",
                            plan.featured ? "text-orange-400" : "text-white/50",
                          )}
                        />
                      </div>
      ...
      {/* ── CTA ───────────────────────────────────────────────────────── */}
      <section className="relative mx-auto mb-32 max-w-5xl px-10 py-24 text-center">
        <div
          className="pointer-events-none absolute -inset-x-20 -inset-y-32 -z-10 [filter:hue-rotate(-30deg)_saturate(1.4)]"
          style={{
            maskImage:
        ...
        <SectionHeading gray="Stop scaffolding," blue="start shipping." />

        <p className="mb-8 text-sm leading-relaxed text-white/40">
          10 free generations on sign up. No credit card required.
          <br />
          Upgrade only once Forge is part of how you build.
        </p>
      ...
      {/* Wrapper to control footer element transitions cleanly on hover */}
