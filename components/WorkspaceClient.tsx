...
export function WorkspaceClient({
  ...
  // Shared across the whole app (see contexts/CreditsContext.tsx) — a
  // decrement here after generation shows up in Header/WorkspaceHeader's
  // CreditsBadge immediately, since they all read the same value.
        ...
        while (true) {
          ...
          for (const line of lines) {
            ...
            try {
              ...
              // Only a genuinely malformed SSE line gets skipped here —
              // this catch no longer has a chance to swallow a real
              // "error" event, since that's handled below, outside it.
              continue;
            }
            ...
            if (event.type === "status") {
              ...
              // Previously thrown inside the JSON.parse try/catch above,
              // where it was caught and discarded by the same handler
              // meant for malformed lines — the request would silently
              // reset with no toast and no assistant reply. Now this
              // throw happens outside that try, so it reaches the outer
              // catch below and actually surfaces to the user.
        ...
        while (true) {
          ...
          for (const line of lines) {
            ...
            try {
              ...
              // Same fix as handleGenerate: only malformed lines get
              // skipped here — a real "error" event is handled below,
              // outside this try, so it can't be silently discarded.
              continue;
            }
